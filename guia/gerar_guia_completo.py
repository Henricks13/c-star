# -*- coding: utf-8 -*-
"""
Gerador de Guia Completo de Recuperação - C-Star
Gera HTML com todos os 45 dias individualizados
"""
...
def gerar_html_completo():
    ...
    dias_protocolo = {
        ...
        **{i: {"medicacoes": [("Vitamina K1 (5mg)", "1 comprimido com refeição", "💊")]} 
           for i in range(1, 5)},
        ...
        ], "compressas_gelo": True, "cirurgia": True},
        
        # Dias 8-9: Pós-cirúrgico imediato
        8: {"medicacoes": [
            ("Vitamina K1 (5mg)", "1 comprimido com refeição", "💊"),
        ...
        ], "compressas_gelo": True, "camara_hiper": True},
        ...
        10: {"medicacoes": [
            ("Cefadroxila", "Antibiótico", "💊"),
            ("Furosemida", "Diurético (pela manhã)", "💊"),
        ], "compressas_gelo": True, "drenagem": "1ª Drenagem Linfática"},
        
        # Dias 11-13: Compressas mornas começam
        **{i: {"medicacoes": [
            ("Cefadroxila", "Antibiótico" if i < 14 else "", "💊"),
            ("Furosemida", "Diurético (pela manhã)" if i < 14 else "", "💊"),
        ], "compressas_mornas": True} for i in range(11, 14)},
        
        # Dia 13: possível 2ª drenagem
        13: {"medicacoes": [
            ("Cefadroxila", "ÚLTIMO DIA - Antibiótico", "💊"),
            ("Furosemida", "ÚLTIMO DIA - Diurético", "💊"),
        ], "compressas_mornas": True, "drenagem": "2ª Drenagem (possível)"},
        
        # Dia 14: RETORNO MÉDICO
        14: {"medicacoes": [], "compressas_mornas": True, "retorno": True, "retirada_pontos": True},
        
        # Dias 15-20: Recuperação
        **{i: {"medicacoes": [], "compressas_mornas": True, "atividade_leve": i >= 15} 
           for i in range(15, 21)},
        
        # Dia 21: 2º RETORNO
        21: {"medicacoes": [], "compressas_mornas": True, "retorno": True, "drenagem": "3ª Drenagem (possível)"},
        
        # Dias 22-45: Apenas compressas mornas e hidratação
        **{i: {"medicacoes": [], "compressas_mornas": True} for i in range(22, 46)},
    }
...
<body>
    ...
    <div class="page cover-page">
        ...
        <div style="display: flex; align-items: center; justify-content: center; gap: 15px; margin-bottom: 80px;">
            <div style="text-align: right;">
                <p style="font-family: 'Playfair Display', serif; font-size: 13pt; color: var(--azul-navy); font-weight: 600; margin: 0; letter-spacing: 1px;">
                    DRA. CAROLINA SANITAGO
                </p>
                <p style="font-size: 9.5pt; color: var(--cinza-claro); margin: 5px 0 0 0;">
                    Fisioterapeuta Especialista
                </p>
            </div>
            <div style="width: 55px; height: 55px; border: 2px solid var(--dourado); border-radius: 50%; display: flex; align-items: center; justify-content: center; background: white;">
                <span style="font-size: 32px; color: var(--dourado);">⚕️</span>
            </div>
        </div>
        
        <!-- Rodapé -->
        <div style="position: absolute; bottom: 40px; left: 0; right: 0; text-align: center;">
            <p style="font-size: 10.5pt; font-weight: 400; color: var(--cinza-claro); margin: 0;">
                Seu roteiro completo para uma recoupação perfeita
            </p>
        </div>
    </div>
...
'''
    ...
    # Gerar dias 1-45
    for dia in range(1, 46):
        dados = dias_protocolo.get(dia, {"medicacoes": [], "compressas_mornas": True})
        
        # Título especial para o dia da cirurgia
        if dia == 7:
            titulo = f"🏥 DIA {dia} ⭐ DIA DA CIRURGIA ⭐"
            subtitle = "O grande dia chegou!"
        elif dia == 14:
            titulo = f"👩‍⚕️ DIA {dia} - RETORNO MÉDICO"
            ...
            titulo = f"📆 DIA {dia}"
            subtitle = f"Preparação Pré-Cirúrgica ({7-dia} dias antes da cirurgia)"
        elif dia <= 14:
            titulo = f"📆 DIA {dia}"
            subtitle = f"Pós-Operatório Imediato ({dia-7}º dia após cirurgia)"
        else:
            titulo = f"📆 DIA {dia}"
            subtitle = f"Fase de Recuperação ({dia-7}º dia após cirurgia)"
...
'''
        ...
        for med_nome, med_detalhe, med_icon in dados.get("medicacoes", []):
            html += f'''
                <tr>
                    <td>____:____</td>
                    <td><span class="icon">{med_icon}</span> {med_nome}</td>
                    <td>{med_detalhe}</td>
...
'''
        ...
        if dados.get("compressas_mornas"):
            html += f'''
                <tr>
                    <td>Ao acordar</td>
...
'''
    ...
    # Adicionar páginas de orientação nutricional
    html += '''
...
if __name__ == "__main__":
    print("Gerando guia completo de 45 dias...")
    html_completo = gerar_html_completo()
    
    with open("Guia_Completo_45_Dias.html", "w", encoding="utf-8") as f:
        f.write(html_completo)
    
    print("✅ Guia gerado com sucesso!")
    print("📄 Arquivo: Guia_Completo_45_Dias.html")
    print("🌟 Total de páginas: 50+ (Capa + 45 dias + Nutrição + Sinais de alerta + Final)")
